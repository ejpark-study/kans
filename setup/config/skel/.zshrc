...
if [[ -f $(which tmux)   ]]; then plugins+=(tmux); fi
if [[ -f $(which npm)    ]]; then plugins+=(npm); fi
if [[ -f $(which docker) ]]; then plugins+=(docker); fi
...
if [[ -f /usr/bin/kubetail.bash ]]; then source /usr/bin/kubetail.bash; fi

# oh my zsh
source $ZSH/oh-my-zsh.sh

# krew
if [[ -f $(which krew) ]]; then export PATH="${KREW_ROOT:-$HOME/.krew}/bin:$PATH"; fi
